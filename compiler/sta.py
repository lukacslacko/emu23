...
class For(Statement):
    ...
    def __str__(self):
        return (
            f"(For) ({self.statement1};{self.cond};{self.statement3}) (do) {self.body}"
        )
...
def statement(line: list[str]) -> Statement:
    ...
    if line[0] == "fun":
        ...
        while len(input_args) > 0:
            ...
            input_type_end = (
                find_first(",", input_args) if "," in input_args else len(input_args)
            )
